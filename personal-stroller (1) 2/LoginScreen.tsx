    ...
    try {
      ...
      if (error) {
        if (error.message.includes("Invalid login credentials")) {
          ...
          throw new Error("Votre compte n'est pas encore confirmé. Vérifiez vos emails.");
        } else if (error.status === 503 || error.status === 500) {
          throw new Error("Le service est temporairement indisponible (Supabase en pause ?).");
        }
        throw error;
      }
    ...
    <div className="w-full flex flex-col bg-navy-900 min-h-screen">
      ...
      <div className="flex-1 flex flex-col justify-center p-8 animate-in">
        <div className="max-w-md w-full mx-auto">
          ...
          <p className="mt-10 text-center text-gray-500 text-xs tracking-wide">
            Pas encore de compte ?{" "}
            <button onClick={onBack} className="text-gold-400 font-bold hover:underline">
              Commencer le quiz
            </button>
          </p>
        </div>
