            ...
            {results.map((item, i) => (
              <div key={i} className="mx-5 glass-card rounded-[2.5rem] overflow-hidden relative flex flex-col group transition-transform duration-500">
                <div className="w-full aspect-[4/5] relative flex items-center justify-center p-8 overflow-hidden bg-navy-900/40">
                  <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[150%] h-[150%] animate-spin-slower opacity-60"><div className="w-full h-full bg-[conic-gradient(var(--tw-gradient-stops))] from-transparent via-gold-400/20 to-transparent blur-3xl"></div></div>
                  <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[120%] h-[120%] animate-spin-slow opacity-30" style={{ animationDirection: 'reverse' }}><div className="w-full h-full bg-[conic-gradient(var(--tw-gradient-stops))] from-transparent via-blue-400/10 to-transparent blur-2xl"></div></div>
                  <div className="absolute inset-0 bg-radial-gradient from-white/10 to-transparent opacity-80"></div>
                  <img src={item.url_image || "https://via.placeholder.com/600x600?text=Image+Indispo"} alt={`${item.marque} ${item.modele}`} className="max-h-full max-w-full object-contain drop-shadow-2xl z-10 transform group-hover:scale-105 transition-transform duration-700 ease-out relative" referrerPolicy="no-referrer" onError={(e) => (e.target as HTMLImageElement).src = "https://via.placeholder.com/600x600?text=Image+Indispo"} />
                </div>
                <div className="px-6 pb-6 pt-4 flex-1 flex flex-col bg-navy-900/40 backdrop-blur-xl border-t border-white/5 relative z-20">
                  <div className="mb-4"><h3 className="text-gold-400 font-bold tracking-[0.2em] text-sm uppercase mb-1 opacity-90">{item.marque}</h3><h2 className="text-3xl font-bold text-white leading-tight">{item.Modele || item.modele}</h2></div>
                  <a href={item['site web'] || item.url_produit || "#"} target="_blank" rel="noopener noreferrer" onClick={() => handleOfferClick(item)} className="mt-auto block w-full py-4 bg-white text-navy-900 text-center font-bold text-lg rounded-2xl shadow-[0_0_20px_rgba(255,255,255,0.2)] hover:shadow-[0_0_30px_rgba(255,255,255,0.4)] active:scale-[0.98] transition-all flex items-center justify-center gap-2">Voir l'offre <ExternalLink size={18} /></a>
                </div>
              </div>
            ))}
          </div>
            ...
            {results.map((item, i) => (
              <div key={i} className="glass-card p-5 rounded-3xl relative flex flex-col transition-all duration-300 hover:-translate-y-2 hover:shadow-[0_15px_40px_rgba(0,0,0,0.4)] hover:border-white/20 group overflow-hidden">
                  <div className="h-64 w-full flex items-center justify-center mb-6 rounded-2xl overflow-hidden bg-navy-900/40 relative">
                      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[200%] h-[200%] animate-spin-slower opacity-50"><div className="w-full h-full bg-[conic-gradient(var(--tw-gradient-stops))] from-transparent via-gold-400/10 to-transparent blur-2xl"></div></div>
                      <div className="absolute inset-0 bg-white/5"></div>
                    <img src={item.url_image || "https://via.placeholder.com/400x400?text=Image+Indispo"} alt={`${item.marque} ${item.modele}`} className="max-h-full max-w-full object-contain drop-shadow-lg transform group-hover:scale-110 transition-transform duration-500 relative z-10" referrerPolicy="no-referrer" onError={(e) => (e.target as HTMLImageElement).src = "https://via.placeholder.com/400x400?text=Image+Indispo"} />
                  </div>
                  <div className="flex-1 flex flex-col relative z-20">
                    ...
                    <div className="mt-auto"><a href={item['site web'] || item.url_produit || "#"} target="_blank" rel="noopener noreferrer" onClick={() => handleOfferClick(item)} className="w-full py-3 bg-gradient-to-r from-gold-400 to-amber-600 text-white rounded-xl font-bold text-sm shadow-lg shadow-gold-400/20 hover:shadow-gold-400/40 transition flex items-center justify-center gap-2 group-hover:scale-[1.02]">VOIR L'OFFRE <ExternalLink size={14} /></a></div>
                  </div>
              </div>
            ))}
          </div>
